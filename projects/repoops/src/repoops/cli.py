...
def persist_run_artifacts(payload: dict[str, object]) -> dict[str, object]:
    ...
    # Track which artifacts were actually written so later steps can distinguish declared outputs
    # from files that already exist on disk.
    persisted_artifacts = list(payload.get("persisted_artifacts", []))
    plan_path_str = str(plan_path.resolve())
    if plan_path_str not in persisted_artifacts:
        persisted_artifacts.append(plan_path_str)
    payload["persisted_artifacts"] = persisted_artifacts

    # Dry-run still writes audit artifacts so the workflow stays inspectable.
